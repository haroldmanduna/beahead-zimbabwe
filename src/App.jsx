...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'SIGNED_IN' && session?.user) {
        ...
        const { data: existing } = await supabase.from('beahead_profiles').select('*').eq('id', session.user.id).single()
        if (!existing) {
          ...
          const tmpl = emailTemplates.welcome(session.user.user_metadata?.full_name || 'there', escrowNum)
          await sendEmail({ to: session.user.email, subject: tmpl.subject, html: tmpl.html, type: 'welcome' })
        }
    ...
    try {
      if (mode === 'signup') {
        ...
        if (data.user) {
          ...
          const tmpl = emailTemplates.welcome(authForm.fullName, escrowNum)
          await sendEmail({ to: authForm.email, subject: tmpl.subject, html: tmpl.html, type: 'welcome' })
          setView('dashboard')
        }
      } else {
  ...
  const createGoal = async () => {
    ...
    const tmpl = emailTemplates.goalCreated(profile?.full_name || 'there', `${data.beahead_cars?.make} ${data.beahead_cars?.model}`, formatUSD(data.goal_amount_usd))
    await sendEmail({ to: user.email, subject: tmpl.subject, html: tmpl.html, type: 'goal_created' })
  ...
  const addDeposit = async (e) => {
    ...
    const tmplRecv = emailTemplates.depositReceived(profile?.full_name || 'there', formatUSD(dep.amount_usd), `${goal.beahead_cars?.make} ${goal.beahead_cars?.model}`, dep.reference_code)
    await sendEmail({ to: user.email, subject: tmplRecv.subject, html: tmplRecv.html, type: 'deposit_received' })
    ...
    <div className="min-h-screen bg-[#fafaf9] text-zinc-900 antialiased selection:bg-zinc-900 selection:text-white">
      ...
      {view === 'auth' && (
        <div className="max-w-[400px] mx-auto px-6 py-16">
          ...
          <div className="bg-white border border-zinc-200 rounded-[20px] p-6">
            <button onClick={handleGoogleAuth} className="w-full bg-white border border-zinc-200 hover:border-zinc-300 hover:bg-zinc-50 py-3 rounded-full text-[13px] font-medium flex items-center justify-center gap-2.5 transition">
            ...
            <div className="flex items-center gap-3 my-5"><div className="h-px bg-zinc-200 flex-1"></div><span className="text-[11px] text-zinc-400">OR</span><div className="h-px bg-zinc-200 flex-1"></div></div>
            <div className="space-y-3">
              ...
              <button onClick={()=>handleAuth('signup')} className="w-full bg-zinc-900 text-white py-3 rounded-full text-[13px] font-medium hover:bg-black transition mt-2">Create account with email</button>
